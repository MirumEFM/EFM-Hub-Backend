...
// TODO: [FIX] Google login timeout
export async function login(page: Page, credentials: LoginData, task: Task) {
  const { email, password } = credentials;

  task.update({ message: "Iniciando login..." });
  ...
  task.update({ message: "Esperando 2FA", progress: 35 });
  ...
  task.update({ status: "finished", message: "Login finalizado!", progress: 100 });
